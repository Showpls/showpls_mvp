...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/tonconnect-manifest.json", (req, res) => {
    try {
      const base =
        process.env.WEBAPP_BASE_URL || `${req.protocol}://${req.get("host")}`;
  ...
  app.post("/api/admin/setup-webapp", async (req, res) => {
    try {
      ...
      const expectedSecret =
        process.env.ADMIN_API_SECRET || process.env.TELEGRAM_WEBHOOK_SECRET;
      ...
      if (!webAppUrl) {
        return res
          .status(400)
          .json({ error: "WEBAPP_BASE_URL or WEBAPP_URL not configured" });
      }
  ...
  app.post("/api/admin/set-webhook", async (req, res) => {
    try {
      ...
      const url =
        providedUrl ||
        (base ? `${base.replace(/\/$/, "")}/telegram/webhook` : undefined);
  ...
  app.get("/api/orders/area", authenticateTelegramUser, async (req, res) => {
    try {
      ...
      const orders = await storage.getNearbyOrders(
        centerLat,
        centerLng,
        radiusKm
      );
  ...
  app.post("/api/orders", authenticateTelegramUser, async (req, res) => {
    try {
      ...
      if (!req.body.isSampleOrder) {
        ...
        const balanceCheck = await tonService.checkSufficientBalance(
          user.walletAddress,
          requiredAmount
        );
        ...
        if (!balanceCheck.sufficient) {
          return res.status(400).json({
            ...
            details: {
              ...
              shortfall: tonService.nanoToTon(
                balanceCheck.required - balanceCheck.balance
              ),
            },
          });
        }
  ...
  app.post("/api/ratings", authenticateTelegramUser, async (req, res) => {
    try {
      ...
      if (!orderId || !toUserId || typeof rating !== "number") {
        return res
          .status(400)
          .json({ error: "orderId, toUserId and numeric rating are required" });
      }
      ...
      const participants = [order.requesterId, order.providerId].filter(
        Boolean
      ) as string[];
      if (!participants.includes(userId) || !participants.includes(toUserId)) {
        return res
          .status(403)
          .json({ error: "Only order participants can rate each other" });
      }
    ...
    async (req, res) => {
      try {
        ...
        if (!userId)
          return res.status(401).json({ error: "User not authenticated" });
        if (!proofUri)
          return res.status(400).json({ error: "Proof URI required" });
        ...
        if (order.providerId !== userId)
          return res
            .status(403)
            .json({ error: "Only provider can deliver order" });
        if (order.status !== "FUNDED")
          return res
            .status(400)
            .json({ error: "Order must be funded before delivery" });
    ...
    async (req, res) => {
      try {
        ...
        if (!user.isProvider)
          return res
            .status(403)
            .json({ error: "Only providers can accept orders" });
        ...
        if (order.requesterId === userId)
          return res
            .status(400)
            .json({ error: "Cannot accept your own order" });
        // Only allow accepting newly created orders; funding will happen afterward
        if (order.status !== "CREATED") {
          return res
            .status(400)
            .json({ error: "Order cannot be accepted in current status" });
        }
        ...
        if (!user.walletAddress) {
          return res
            .status(400)
            .json({
              error: "Provider wallet connection required to accept orders",
            });
        }
    ...
    async (req, res) => {
      try {
        ...
        if (!user || !user.telegramId) {
          return res
            .status(400)
            .json({ error: "No Telegram ID found for user" });
        }
